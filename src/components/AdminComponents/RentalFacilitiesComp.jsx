...
//         if (formData.maximum_jumpers && formData.maximum_jumpers < 1) {
//       errors.maximum_jumpers = 'Minimum jumpers must be at least 1';
//     }
...
//         maximum_jumpers: parseInt(formData.maximum_jumpers),
//       };
...
//       maximum_jumpers: facility.maximum_jumpers || '',
...
//                 <input
//                   type="number"
//                   name="maximum_jumpers"
//                   value={formData.maximum_jumpers}
//                   onChange={handleInputChange}
//                   min="1"
//                   className={`w-full rounded-md border ${fieldErrors.maximum_jumpers ? 'border-red-300' : 'border-gray-300'} px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500`}
//                   placeholder="Minimum jumpers required"
//                 />
//                 {fieldErrors.maximum_jumpers && (
//                   <p className="mt-1 text-sm text-red-600">{fieldErrors.maximum_jumpers}</p>
//                 )}
//               </div>
...
const RentalFacilitiesComp = () => {
  ...
  // Form data state - updated to match backend model
  const [formData, setFormData] = useState({
    rental_jumper_group: '',
    rental_group_name: '',
    per_jumper_price: '',
  ...
  const validateForm = () => {
    ...
    if (formData.maximum_jumpers && formData.maximum_jumpers < 1) {
      errors.maximum_jumpers = 'Maximum jumpers must be at least 1';
    }

    if (formData.minimum_jumpers && formData.maximum_jumpers) {
      if (parseInt(formData.minimum_jumpers) > parseInt(formData.maximum_jumpers)) {
        errors.maximum_jumpers = 'Maximum jumpers must be greater than or equal to minimum jumpers';
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Prepare data for submission - updated to match backend model
      const submitData = {
        rental_jumper_group: formData.rental_jumper_group,
        rental_group_name: formData.rental_group_name || null,
        per_jumper_price: parseFloat(formData.per_jumper_price).toFixed(2),
        minimum_jumpers: parseInt(formData.minimum_jumpers),
        maximum_jumpers: parseInt(formData.maximum_jumpers),
  ...
  const handleEdit = (facility) => {
    ...
    setFormData({
      rental_jumper_group: facility.rental_jumper_group || '',
      rental_group_name: facility.rental_group_name || '',
      per_jumper_price: facility.per_jumper_price || '',
      minimum_jumpers: facility.minimum_jumpers || '',
      maximum_jumpers: facility.maximum_jumpers || '',
      inclusions: facility.inclusions || ''
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        <div className="flex items-start justify-between">
                          <div>
                            <h3 className="text-xl font-semibold text-gray-900">{facility.rental_jumper_group}</h3>
                            {facility.rental_group_name && (
                              <p className="text-gray-600 mt-1">{facility.rental_group_name}</p>
                            )}
                            <div className="flex items-center space-x-4 mt-2">
                              <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                ${perJumperPrice} per jumper
                              ...
                              <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                Max. {facility.maximum_jumpers} jumpers
                              </span>
                            </div>
                          </div>
                        </div>
        ...
        <div className="bg-white rounded-lg shadow-lg border border-gray-200 p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Rental Group Name (Optional)
                </label>
                <input
                  type="text"
                  name="rental_group_name"
                  value={formData.rental_group_name}
                  onChange={handleInputChange}
                  className="w-full rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                  placeholder="e.g., Standard Group, Premium Group"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  min="1"
                  className={`w-full rounded-md border ${fieldErrors.maximum_jumpers ? 'border-red-300' : 'border-gray-300'} px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500`}
                  placeholder="Maximum jumpers allowed"
                />
                {fieldErrors.maximum_jumpers && (
                  <p className="mt-1 text-sm text-red-600">{fieldErrors.maximum_jumpers}</p>
                )}
