...
// // Hi see that code there are some bugs. you have to write the compelate code line by line word by word without misssing a single feature.
// // The bugs 1) If i am try to enter the jump pass then its not allowed even the shudule is avaibale on that days. 
// // Like I try to Add New Jump Pass that is Schedule with (Open Jump) from (Monday to Friday)  and allowed time is 120 mins
// // But the conflict will come (
...
// // Jump Time: 120 minutes
// // Schedule Not Available
// // High Priority
// // Open Jump is not available on Tuesday

// // Schedule Not Available
// // High Priority
// // Open Jump is not available on Wednesday

// // Schedule Not Available
// // High Priority
// // Open Jump is not available on Thursday

// // Schedule Not Available
// // High Priority
// // Open Jump is not available on Friday
// // )


// // Now how you see the conflict. First you have to see the special hours if any special hours is avaible then you give confict or allowed on that speical hours based
// // If there is not speaclal hours then you see the regular hours. then you give conflict or allowed


// // and one more change I want if tax included is yes (then you keep it simple) but i user select No then show the input box to enter how much tax then show the tax calcuatoin
// // and if user select no then he must have to enter 1 or + any numebr = 0 is not allowed but 0.1+ every number is allowed till 100
...
// priority take form database
// schedule selction multpite
// and one more flag can custom take a part in 1 more multiple
// Age Allowed = text  (donot allowed number)
// add one more option in jump time allowed (all day)
// I donot have to calculate the Tax i just have to write yes and no



// ok now I need few modification. you have to write the compleate code line by line with out missing a single feature.
// 1) I want you keep the priority as a selection tab.  but first you have to get the highest priority number from the databbase form the priority coloum
//   then you give 4 selectoin options.
//   like if there is not priority for location_id (1) its mean priority is = 0 so you have to display as selection of 1 , 2 , 3, 4 + (Donot pitch its value is 999)
//   and if there is a highest priority is 2 and 1 also exist in the prioirty thsi show the (3, 4, 5, 6) + Donot pitch its value is 999
//   and if there is a highteset priorit is 2 but 1 is not exist in the database then you have to show (1,3, 4,5) + Donot pitch its value is 999


// 2)  ok I  want you change the age to text input from selection and i want a persion just add a text (number are not allowed like 1,2,3...) just abc is allowed. in age input box

// 3) we Donot have to calculate the Tax we just have to set the Yes and no. we donot need any tax % or calculation. just yes or no
// 4) this is the mager update. Now I want a user is able to select multple option schedulle_with
//    And the conflict is just or (like a jump pass that is from monday to sunday) and a person select 2 schedulle_with like (open_jump and glow). and open_jump avaiable form (monday to friday) and glow avaiable from (satrtidau and sunday) then you donot show any conflict. 
// you allowd add on more check bot that is (custom take a part in 1 more multiple)
// and i also have to add one more coloum in backend. database table. for that (will custom able to take part in more then 1 schedules or not) also tell me what name you used in fronted that i add same coloum name in the backend model database.

// Now you have to write compleate code line by line. and donot miss a sinlge feature from existing code. 
...
//     // FIXED: Enhanced conflict detection that checks both special hours and regular hours
//     const detectConflicts = (newPass, existingHours) => {
//         const conflicts = [];
...
//         daysInRange.forEach(day => {
//             // First, check for special hours on this day
...
//             // If there are special hours, use them for conflict checking
...
//                         conflicts.push({
//                             type: 'TIME_EXCEEDS_AVAILABILITY',
//                             message: `Jump time (${newPass.jump_time_allowed}) exceeds available special hours time for ${formatScheduleWith(newPass.schedule_with)} on ${day}`,
//                             day: day,
//                             availableTime: `${formatTime(hour.start_time)} - ${formatTime(hour.end_time)}`,
//                             availableMinutes: availableMinutes,
//                             requestedMinutes: jumpTimeMinutes,
...
//                 });
//             } 
//             // If no special hours, check regular hours
...
//                         conflicts.push({
//                             type: 'TIME_EXCEEDS_AVAILABILITY',
//                             message: `Jump time (${newPass.jump_time_allowed}) exceeds available regular hours time for ${formatScheduleWith(newPass.schedule_with)} on ${day}`,
//                             day: day,
//                             availableTime: `${formatTime(hour.start_time)} - ${formatTime(hour.end_time)}`,
//                             availableMinutes: availableMinutes,
//                             requestedMinutes: jumpTimeMinutes,
...
//                 });
//             }
//             // If no hours at all for this schedule on this day
//             else {
//                 conflicts.push({
//                     type: 'SCHEDULE_NOT_AVAILABLE',
//                     message: `${formatScheduleWith(newPass.schedule_with)} is not available on ${day}`,
...
//                 ...prev,
//                 [name]: value,
//                 // Reset tax percentage when switching to 'yes'
//                 tax_percentage: value === 'yes' ? '' : prev.tax_percentage
//             }));
//         } else {
//             setFormData(prev => ({
//                 ...prev,
...
//         // FIXED: Enhanced tax validation
//         if (formData.tax_included === 'no' && formData.tax_percentage) {
//             if (formData.tax_percentage <= 0) {
//                 errors.tax_percentage = 'Tax percentage must be greater than 0';
//             } else if (formData.tax_percentage > 100) {
//                 errors.tax_percentage = 'Tax percentage cannot exceed 100';
//             }
//         }

//         // FIXED: Require tax percentage when tax_included is 'no'
//         if (formData.tax_included === 'no' && (!formData.tax_percentage || formData.tax_percentage === '')) {
//             errors.tax_percentage = 'Tax percentage is required when tax is not included';
//         }
...
//             const submitData = { 
//                 ...formData,
//                 // Convert tax_included to boolean for backend
...
//             // Convert tax_percentage to string for backend
//             if (submitData.tax_percentage) {
//                 submitData.tax_percentage = submitData.tax_percentage.toString();
//             }
...
//     const calculateFinalPrice = (price, taxPercentage, taxIncluded) => {
//         if (!price) return '0.00';
//         const priceNum = parseFloat(price);
//         const taxNum = parseFloat(taxPercentage || 0);

//         if (taxIncluded === 'yes') {
//             // If tax is included, the price is the final price
//             return priceNum.toFixed(2);
//         } else {
//             // If tax is not included, add tax to the base price
//             return (priceNum + (priceNum * taxNum / 100)).toFixed(2);
//         }
//     };
...
//                         .map((pass) => {
//                             const isTaxIncluded = pass.tax_included;
//                             const finalPrice = calculateFinalPrice(pass.price, pass.tax_percentage, isTaxIncluded ? 'yes' : 'no');
...
//                                                         No ({pass.tax_percentage}% tax added)
//                                                     </span>
//                                                 )}
//                                             </p>
//                                         </div>
...
//                                                 <p className="font-medium text-green-600">
//                                                     ${finalPrice}
//                                                     {!isTaxIncluded && pass.tax_percentage && (
//                                                         <span className="text-xs text-gray-500 ml-1">
//                                                             (incl. {pass.tax_percentage}% tax)
//                                                         </span>
//                                                     )}
...
//                             {/* FIXED: Show tax percentage input for BOTH 'yes' and 'no' options */}
//                             <div>
//                                 <label className="block text-sm font-medium text-gray-700 mb-2">
//                                     Tax Percentage {formData.tax_included === 'no' && '*'}
//                                 </label>
//                                 <div className="relative">
//                                     <input
//                                         type="number"
...
//                                         }`}
//                                         placeholder="0.00"
//                                         disabled={formData.tax_included === 'yes'}
//                                     />
//                                     <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
//                                         <span className="text-gray-500 sm:text-sm">%</span>
//                                     </div>
...
//                                 {formData.tax_included === 'yes' && (
//                                     <p className="mt-1 text-xs text-gray-500">
//                                         Tax percentage is calculated automatically when tax is included
//                                     </p>
//                                 )}
//                                 {formData.tax_included === 'no' && (
//                                     <p className="mt-1 text-xs text-gray-500">
//                                         Enter tax percentage to be added to the base price (0.1 - 100)
//                                     </p>
//                                 )}
//                             </div>
...
//                                             <span className="text-gray-600">Final Price:</span>
//                                             <p className="font-medium text-green-600">
//                                                 ${calculateFinalPrice(formData.price, formData.tax_percentage, formData.tax_included)}
//                                                 {formData.tax_included === 'no' && formData.tax_percentage ? (
//                                                     <span className="text-xs text-gray-500 ml-1">
//                                                         (base + {formData.tax_percentage}% tax)
//                                                     </span>
//                                                 ) : formData.tax_included === 'yes' ? (
//                                                     <span className="text-xs text-gray-500 ml-1">
//                                                         (tax included)
...
// now there are few bugs. you just have to tell me what i have to chagne or update and in which function. and if possible then write the compleate function i just replace it
// Now the bug is about the valid Days. If i just select 1 schedule with like (Litle Leaper from (monday to friday)) Then its show that confilicts = = 

// Schedule Not Available
// High Priority
// Little Leaper is not available on Tuesday

// Schedule Not Available
// High Priority
// Little Leaper is not available on Wednesday

// Schedule Not Available
// High Priority
// Little Leaper is not available on Thursday

// Schedule Not Available
// High Priority
// Little Leaper is not available on Friday


// But the Little Leaper is avaible in regularhours from (monday to Firday)
// But if i add the day from (monday to monday) then its added successuflly



// and same is happend with the multipele selection if i select (2 then its show that error )
...
// New Jump Pass:
// Schedule: Little Leaper, Open Jump

// Days: Monday to Friday

// Jump Time: 90 minutes
...
const JumpPassComp = () => {
    ...
    // Generate priority options based on existing priorities
    const generatePriorityOptions = (existingPasses) => {
        ...
        const highestPriority = Math.max(...existingPriorities);
        const allNumbers = Array.from({ length: 20 }, (_, i) => i + 1);

        // Find the first 4 available numbers that don't exist in current priorities
        const availableOptions = [];
        for (let i = 1; i <= 20 && availableOptions.length < 4; i++) {
            if (!existingPriorities.includes(i)) {
                availableOptions.push(i);
            }
        }

        // If we don't have 4 options, add numbers beyond the highest priority
        if (availableOptions.length < 4) {
            for (let i = highestPriority + 1; availableOptions.length < 4; i++) {
                if (!existingPriorities.includes(i)) {
                    availableOptions.push(i);
                }
            }
        ...
        return availableOptions.slice(0, 4);
    };
    ...
    // Enhanced conflict detection for multiple schedule_with
    // const detectConflicts = (newPass, existingHours) => {
    //     const conflicts = [];
    ...
    //     // If jump time is "all day", skip time-based conflict checking
    //     const isAllDay = newPass.jump_time_allowed.toLowerCase() === 'all day';

    //     daysInRange.forEach(day => {
    //         let hasAvailableSchedule = false;
    ...
    //                 specialHours.forEach(hour => {
    //                     if (isAllDay) {
    //                         // For all day, just mark as available
    ...
    //                                 type: 'TIME_EXCEEDS_AVAILABILITY',
    //                                 message: `Jump time (${newPass.jump_time_allowed}) exceeds available special hours time for ${formatScheduleWith(schedule)} on ${day}`,
    //                                 day: day,
    //                                 schedule: schedule,
    //                                 availableTime: `${formatTime(hour.start_time)} - ${formatTime(hour.end_time)}`,
    //                                 availableMinutes: availableMinutes,
    ...
    //                             scheduleConflicts.push({
    //                                 type: 'TIME_EXCEEDS_AVAILABILITY',
    //                                 message: `Jump time (${newPass.jump_time_allowed}) exceeds available regular hours time for ${formatScheduleWith(schedule)} on ${day}`,
    //                                 day: day,
    //                                 schedule: schedule,
    //                                 availableTime: `${formatTime(hour.start_time)} - ${formatTime(hour.end_time)}`,
    //                                 availableMinutes: availableMinutes,
    ...
    //         // If no schedule has available time for this day, add conflicts
    //         if (!hasAvailableSchedule && scheduleConflicts.length > 0) {
    ...
    // Fixed conflict detection for multiple schedule_with
    ...
    const detectConflicts = (newPass, existingHours) => {
        ...
        // For each day in range, check if at least one selected schedule is available
        daysInRange.forEach(day => {
            newPass.schedule_with.forEach(schedule => {
                ...
                // Check if we have any hours (special or regular) for this schedule on this day
                if (specialHours.length > 0) {
                ...
                // If no hours found for this schedule on this day, add conflict
                ...
                // If hours found but jump time exceeds available time (and not all day)
                else if (!isAllDay && jumpTimeMinutes > availableMinutes) {
                    conflicts.push({
                        type: 'TIME_EXCEEDS_AVAILABILITY',
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                >
                                    ...
                                    <option value="999">Do not pitch</option>
                                </select>
                                {fieldErrors.jump_pass_priority && (
                                    <p className="mt-1 text-sm text-red-600">{fieldErrors.jump_pass_priority}</p>
                                )}
