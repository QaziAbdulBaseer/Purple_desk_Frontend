...
// // there 2 problems you have to write the compleate code line by line and word by word
// // now there is also a problem with model.  when I delect any conflict then its not update on the time. mean its not refresh the model. still the old conflict will display. 
// // And also the model will not return back on same form if i click edit and edit that form then model will not return. 
// // we are not able to increas or decreased the Data Range of special Hours.
// // when we open the edio start date will show in the ending date. and not able to increased or decreased the Date range the conflict will display
// // Ok now the we are not able to increased to decreased the Date Range in special hours like i have a special hours from (03/11/2025 to 06/11/2025)  now i click on the ediot button. and want to increaed its date from 06/11/2025 tp 8/11/2025  mean now i want (03/11/2025 to 08/11/2025)  but its show that type of conflict == = Special Hours
// // Medium Priority
// // 04/11/2025 to 04/11/2025
// // Time overlap: 7:42 AM - 9:42 AM
...
// // model will not return where is needed.


// // i have some bugs. you have to write the compleate code line by line and word by word
// // the problem in Early Closing , late openinng, early opening. etc. 
// // The problem is if i have early Closing from (03/11/2025 to 07/11/2025)  and I want to closed the barncd on the 05/11/2025 then its give the confilct for all 4 days and asked to deleate or ediot all 4 days = =Cannot mark as closed because there are existing early_closing hours 
// // so i want to store multip rows with same (same_entry_id) for early closing , late closing and also for the closed
// // same as we are doing with the special hours. 
// //now the problems WE are not able to deleate the speical hours in one click those where are enter with same enter id.  i want when i click the main delete button then its delete all in once. 





// // i need few more changing. You have to write teh compleate code line by line and word by word without missing a single feature.
// // One-click deletion: Deleting any entry in a group deletes all entries with the same same_entry_id  i want if i click the main delete button that is with edit button then its deleate all. but if any one deleate jsut one row then the date are divided in on 2 parts. like i have a special hours form 10/3/2025 to 17/3/2025 and i deleate 13/3/2025 then deleate that one and give the same_entry id to (10/3/2025 to 12/3/2025) and give same entry_id ot (14/3/2025 to 17/3/2025)
// // I want you update that part on the backend if there is any special hours or any closed or early closed open etc. that is past date. so you deleate that from data base. 
// // Like on (22/10/2025) I add the branch is closed on (22/10/2025)  and today is 24/10/2025 so you deleate the 22/10/2025 row from the database and manage that in backend
...
//     // Enhanced conflict detection function - FIXED to handle early closing conflicts properly
//     const detectConflicts = (newHour, existingHours, editingId = null) => {
//         const conflicts = [];
//         const processedConflictIds = new Set(); // Track processed conflicts to avoid duplicates
...
//                 // Create a unique identifier for this conflict to avoid duplicates
//                 const conflictId = `${existing.hours_of_operation_id}-${newHour.hours_type}-${existing.hours_type}`;
                
//                 // Skip if we've already processed this conflict
//                 if (processedConflictIds.has(conflictId)) return;
...
//                 // FIX: Check for special vs closing/opening conflicts FIRST to avoid duplicates
...
//                             message: timeConflict, // Use the specific time conflict message
//                             conflictingHour: existing,
//                             severity: 'high',
//                             conflictId: conflictId
//                         });
//                         return; // Skip other conflict checks for this pair
//                     }
//                 }

//                 // Time-based conflicts for non-closed types (only if not already handled by special case)
//                 if (newHour.hours_type !== 'closed' && existing.hours_type !== 'closed') {
...
//                 // FIX: Check for early/late closing conflicts with any hours that extend beyond closing time
//                 if ((existing.hours_type === 'early_closing' || existing.hours_type === 'late_closing') && 
//                     newHour.hours_type !== 'closed' && newHour.hours_type !== 'regular') {
                    
//                     const closingConflict = checkClosingConflict(existing, newHour);
//                     if (closingConflict) {
//                         conflicts.push({
//                             type: 'HOURS_AFTER_CLOSING',
//                             message: closingConflict,
//                             conflictingHour: existing,
//                             severity: 'high',
//                             conflictId: conflictId
//                         });
...
//                 // FIX: Check for early/late opening conflicts with any hours that start before opening time
//                 if ((existing.hours_type === 'early_opening' || existing.hours_type === 'late_opening') && 
//                     newHour.hours_type !== 'closed' && newHour.hours_type !== 'regular') {
                    
//                     const openingConflict = checkOpeningConflict(existing, newHour);
//                     if (openingConflict) {
//                         conflicts.push({
//                             type: 'HOURS_BEFORE_OPENING',
//                             message: openingConflict,
//                             conflictingHour: existing,
//                             severity: 'high',
//                             conflictId: conflictId
//                         });
...
//     // NEW: Check for hours that extend beyond closing time
//     const checkClosingConflict = (closingHour, otherHour) => {
//         if (!closingHour.end_time || !otherHour.start_time || !otherHour.end_time) return null;
...
//         const closingTime = timeToMinutes(closingHour.end_time);
//         const otherStart = timeToMinutes(otherHour.start_time);
//         const otherEnd = timeToMinutes(otherHour.end_time);

//         // If the other hours extend beyond the closing time, it's a conflict
//         if (otherEnd > closingTime) {
//             return `Cannot add ${otherHour.hours_type} hours that extend beyond ${formatTime(closingHour.end_time)} closing time`;
//         }

//         return null;
//     };

//     // NEW: Check for hours that start before opening time
//     const checkOpeningConflict = (openingHour, otherHour) => {
//         if (!openingHour.start_time || !otherHour.start_time || !otherHour.end_time) return null;
...
//         const openingTime = timeToMinutes(openingHour.start_time);
//         const otherStart = timeToMinutes(otherHour.start_time);
//         const otherEnd = timeToMinutes(otherHour.end_time);

//         // If the other hours start before the opening time, it's a conflict
//         if (otherStart < openingTime) {
//             return `Cannot add ${otherHour.hours_type} hours that start before ${formatTime(openingHour.start_time)} opening time`;
//         }

//         return null;
//     };

//     // NEW: Check day overlap for regular hours
...
//         return !(hour1EndIndex < hour2StartIndex || hour1StartIndex > hour2EndIndex);
//     };

//     // Check time conflicts between two hours - ENHANCED to handle partial times
...
//                 // Conflict if closing time is during special hours
...
//                 // Conflict if opening time is during special hours
...
//             try {
//                 const deleteSuccess = await handleDelete(conflictHour.hours_of_operation_id);
...
//                 // ADDED: For opening types, ensure ending_date is same as starting_date if not provided
...
//     // Delete hour - Updated to refresh data after deletion
...
//                 // ADDED: Force a state update to ensure UI refreshes
//                 setHours(prev => [...prev]);
...
//                                                             </button>
//                                                             <button
//                                                                 onClick={() => handleDelete(firstHour.hours_of_operation_id)}
...
//                                                         {group.entries.length === 1 && group.hours_type !== 'regular' && firstHour.starting_date && (
//                                                             <div className="flex justify-between">
...
// // now i have few more bugs. You have to write the compleate code line by line and word by word. without skipping a single word

// // 1) The problem is The regulare hours will not show the conflict. like I have a special hours from (monday to friday from (2pm to 6pm) , and I enter new hours (from monday to friday from (4pm to 8pm ), then i has to stop me and tell me i have a conflict. ))
// // 2) kindly show the days in with time in the regulra hours div. (where we are showing time, schedule with and ages) I also need days names
// // 
...
// //     // Group hours by same_entry_id for display
// //     const groupHoursBySameEntryId = (hoursList) => {
// //         const grouped = {};
...
// //     // Enhanced conflict detection function - FIXED for multi-day entries
// //     const detectConflicts = (newHour, existingHours, editingId = null) => {
// //         const conflicts = [];
...
// //                 // FIX: For multi-day entries, only show conflict if the specific date conflicts
// //                 // Check if we're dealing with the exact same date for single-day conflicts
// //                 const isSameDateConflict = newStartDate && existingStartDate && 
// //                     newStartDate.toDateString() === existingStartDate.toDateString();

// //                 // Conflict: Trying to close when other hours exist
// //                 if (newHour.hours_type === 'closed' && existing.hours_type !== 'closed' && isSameDateConflict) {
// //                     conflicts.push({
// //                         type: 'CLOSED_WITH_SPECIAL_HOURS',
...
// //                 if (isSpecialVsClosing && isSameDateConflict) {
// //                     const specialHour = newHour.hours_type === 'special' ? newHour : existing;
// //                     const closingHour = newHour.hours_type !== 'special' ? newHour : existing;
...
// //     // Check day overlap for regular hours
// //     const checkDayOverlap = (hour1, hour2) => {
// //         if (hour1.hours_type !== 'regular' || hour2.hours_type !== 'regular') return false;
...
// //         return new Promise(async (resolve, reject) => {
// //             const confirmMessage = deleteGroup 
// //                 ? 'Are you sure you want to delete this entire group?'
// //                 : 'Are you sure you want to delete this single day?';
...
// //                 setSuccess(deleteGroup ? 'Group deleted successfully!' : 'Day deleted successfully!');
// //                 setTimeout(() => setSuccess(''), 3000);
// //                 resolve(true);
// //             } catch (err) {
// //                 setError(err.message);
...
// //             }));
// //         }
// //         // Reset start and end times when changing to early closing/opening types
// //         else if (name === 'hours_type' && (value === 'early_closing' || value === 'late_closing' || value === 'early_opening' || value === 'late_opening')) {
// //             setFormData(prev => ({
// //                 ...prev,
// //                 [name]: value,
// //                 schedule_with: '',
// //                 ages_allowed: '',
// //                 start_time: value.includes('closing') ? '' : prev.start_time,
// //                 end_time: value.includes('opening') ? '' : prev.end_time
// //             }));
// //         } else {
// //             setFormData(prev => ({
// //                 ...prev,
...
// //                                                         {/* Show single date for single day entries */}
// //                                                         {!isMultiDayGroup && firstHour.starting_date && (
// //                                                             <div className="flex justify-between">
// //                                                                 <span className="text-gray-600">Date:</span>
// //                                                                 <span className="font-medium">
// //                                                                     {new Date(firstHour.starting_date).toLocaleDateString()}
...
// //                                                         {/* Show individual days for multi-day groups */}
// //                                                         {isMultiDayGroup && (
// //                                                             <div className="border-t pt-2 mt-2">
// //                                                                 <h4 className="text-sm font-medium text-gray-700 mb-1">Individual Days:</h4>
// //                                                                 <div className="space-y-1 max-h-20 overflow-y-auto">
