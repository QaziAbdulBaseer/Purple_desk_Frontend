...
const BirthDayPackagesComp = () => {
  ...
  const [isCustomPartyRoomTime, setIsCustomPartyRoomTime] = useState(false);
  const [customPartyRoomTime, setCustomPartyRoomTime] = useState('');

  // Form data state initialization with ALL fields from database including new fields
  ...
  const JUMP_TIME_OPTIONS = [
    '60 minutes', '90 minutes', '120 minutes', '150 minutes', '180 minutes', 'custom'
  ];

  const PARTY_ROOM_TIME_OPTIONS = [
    '30 minutes', '45 minutes', '60 minutes', '75 minutes', '90 minutes', 'custom'
  ];
  ...
  // Handle custom jump time selection
  const handleJumpTimeSelect = (value) => {
    if (value === 'custom') {
      setIsCustomJumpTime(true);
      setFormData(prev => ({
        ...prev,
        jump_time: customJumpTime || ''
      }));
    } else {
  ...
  // Handle custom party room time selection
  const handlePartyRoomTimeSelect = (value) => {
    if (value === 'custom') {
      setIsCustomPartyRoomTime(true);
      setFormData(prev => ({
        ...prev,
        party_room_time: customPartyRoomTime || ''
      }));
    } else {
      setIsCustomPartyRoomTime(false);
      setFormData(prev => ({
        ...prev,
        party_room_time: value
      }));
    }
  };
  ...
  // Handle custom party room time input change
  const handleCustomPartyRoomTimeChange = (e) => {
    const value = e.target.value;
    setCustomPartyRoomTime(value);
    setFormData(prev => ({
      ...prev,
      party_room_time: value
    }));

    if (fieldErrors.party_room_time) {
      setFieldErrors(prev => ({
        ...prev,
        party_room_time: ''
      }));
  ...
  // Handle input changes for all field types - UPDATED: Fix balloon package logic
  ...
  // Validate form data - UPDATED: Remove required validation for balloon package
  const validateForm = () => {
    const errors = {};
    ...
    // Validate new count fields
    if (formData.food_included_count && formData.food_included_count < 0) {
      errors.food_included_count = 'Food included count cannot be negative';
    }

    if (formData.drinks_included_count && formData.drinks_included_count < 0) {
      errors.drinks_included_count = 'Drinks included count cannot be negative';
    }

    // REMOVED: Validation for balloon package - now optional
    // Only validate credit if provided
    if (formData.balloon_package_included) {
      if (formData.credit && formData.credit < 0) {
        errors.credit = 'Credit amount cannot be negative';
      }

      // REMOVED: Validation for free balloon package requiring a balloon package
      // Now it's optional even if marked as free
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const submitData = {
        ...
        // Handle new count fields
        food_included_count: formData.food_included_count ? parseInt(formData.food_included_count) : null,
        drinks_included_count: formData.drinks_included_count ? parseInt(formData.drinks_included_count) : null,
        // NEW FIELD: include the free balloon package field
        is_any_balloon_package_is_free: formData.is_any_balloon_package_is_free,
        // Balloon package is optional, send null if not selected
        balloon_party_package: formData.balloon_party_package || null
      };
  ...
  const handleEdit = (pkg) => {
    ...
    // Check if Jump Time value is custom
    const isJumpTimeCustom = !JUMP_TIME_OPTIONS.includes(pkg.jump_time) && pkg.jump_time !== 'custom';

    // Check if Party Room Time value is custom
    const isPartyRoomTimeCustom = !PARTY_ROOM_TIME_OPTIONS.includes(pkg.party_room_time) && pkg.party_room_time !== 'custom';
    ...
    if (isJumpTimeCustom) {
      setIsCustomJumpTime(true);
      setCustomJumpTime(pkg.jump_time);
    } else {
      setIsCustomJumpTime(false);
      setCustomJumpTime('');
    }

    if (isPartyRoomTimeCustom) {
      setIsCustomPartyRoomTime(true);
      setCustomPartyRoomTime(pkg.party_room_time);
    } else {
      setIsCustomPartyRoomTime(false);
      setCustomPartyRoomTime('');
    }
    ...
    // Populate ALL fields from the database including new fields
    setFormData({
      ...
      jump_time: isJumpTimeCustom ? pkg.jump_time : (pkg.jump_time || ''),
      party_room_time: isPartyRoomTimeCustom ? pkg.party_room_time : (pkg.party_room_time || ''),
      ...
      // NEW FIELDS
      party_environment_name: pkg.party_environment_name || '',
      food_included_count: pkg.food_included_count || '',
      drinks_included_count: pkg.drinks_included_count || '',
      perks_for_guest_of_honor: pkg.perks_for_guest_of_honor || ''
    });
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex items-start space-x-4 flex-1">
                        ...
                        <div className="flex-1">
                          ...
                          {/* New Fields Summary */}
                          {(pkg.party_environment_name || pkg.food_included_count || pkg.drinks_included_count) && (
                            <div className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                              {pkg.party_environment_name && (
                                <div>
                                  <span className="text-gray-600 text-xs">Environment:</span>
                                  <p className="font-medium">{pkg.party_environment_name}</p>
                                </div>
                              )}
                              {pkg.food_included_count && (
                                <div>
                                  <span className="text-gray-600 text-xs">Food Included:</span>
                                  <p className="font-medium">{pkg.food_included_count} items</p>
                                </div>
                              )}
                              {pkg.drinks_included_count && (
                                <div>
                                  <span className="text-gray-600 text-xs">Drinks Included:</span>
                          ...
                          {pkg.balloon_package_included && (
                            <div className="mt-3 p-3 bg-pink-50 rounded-md border border-pink-200">
                              <div className="flex items-center space-x-2">
                                <span className="text-pink-600">🎈</span>
                                <span className="text-sm font-medium text-pink-800">
                                  Balloon Package: {balloonPackageName || 'No specific package selected'}
                                </span>
                                {pkg.promotion_code && (
                                  <span className="text-xs text-pink-600">
                                    | Promotion: {pkg.promotion_code}
                    ...
                    <div className="border-t border-gray-200 p-6 bg-gray-50 rounded-b-lg">
                      <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                          ...
                          {pkg.drinks_included_count && (
                            <div>
                              <span className="text-gray-600">Drinks Included:</span>
                              <p className="font-medium">{pkg.drinks_included_count} items</p>
                            </div>
                          )}
                        ...
                        {pkg.perks_for_guest_of_honor && (
                          <div className="p-4 bg-purple-50 rounded-lg border border-purple-200">
                            <h4 className="font-semibold text-purple-800 mb-3">Perks for Guest of Honor</h4>
                            <p className="font-medium text-purple-700">{pkg.perks_for_guest_of_honor}</p>
                          </div>
                        )}
        ...
        <div className="bg-white rounded-lg shadow-lg border border-gray-200 p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                {!isCustomJumpTime ? (
                  ...
                  <div className="flex space-x-2">
                    <input
                      ...
                      className={`flex-1 rounded-md border ${fieldErrors.jump_time ? 'border-red-300' : 'border-gray-300'} px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500`}
                      placeholder="Enter custom jump time"
                    />
                    <button
                      type="button"
                      onClick={() => {
              ...
              {/* Party Room Time with Custom Option */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Party Room Time *
                </label>
                {!isCustomPartyRoomTime ? (
                  <select
                    value={formData.party_room_time}
                    onChange={(e) => handlePartyRoomTimeSelect(e.target.value)}
                  ...
                  <div className="flex space-x-2">
                    <input
                      type="text"
                      value={customPartyRoomTime}
                      onChange={handleCustomPartyRoomTimeChange}
                      className={`flex-1 rounded-md border ${fieldErrors.party_room_time ? 'border-red-300' : 'border-gray-300'} px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500`}
                      placeholder="Enter custom party room time"
                    />
                    <button
                      type="button"
                      onClick={() => {
              ...
              {/* NEW FIELDS: Party Environment, Food Count, Drinks Count */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Party Environment Name
                </label>
                <input
                  type="text"
                  name="party_environment_name"
                  value={formData.party_environment_name}
                  onChange={handleInputChange}
                  className="w-full rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                  placeholder="e.g., Jungle Theme, Space Adventure"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  value={formData.food_included_count}
                  onChange={handleInputChange}
                  min="0"
                  className={`w-full rounded-md border ${fieldErrors.food_included_count ? 'border-red-300' : 'border-gray-300'} px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500`}
                  placeholder="Number of food items"
                />
                {fieldErrors.food_included_count && (
                  <p className="mt-1 text-sm text-red-600">{fieldErrors.food_included_count}</p>
                )}
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Drinks Included Count
                </label>
                <input
                  type="number"
                  name="drinks_included_count"
                  value={formData.drinks_included_count}
                  onChange={handleInputChange}
                  min="0"
                  className={`w-full rounded-md border ${fieldErrors.drinks_included_count ? 'border-red-300' : 'border-gray-300'} px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500`}
                  placeholder="Number of drink items"
                />
                {fieldErrors.drinks_included_count && (
                  <p className="mt-1 text-sm text-red-600">{fieldErrors.drinks_included_count}</p>
                )}
              </div>
              ...
              {/* NEW FIELD: Perks for Guest of Honor */}
              <div className="md:col-span-3">
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Perks for Guest of Honor
                </label>
                <textarea
                  name="perks_for_guest_of_honor"
                  value={formData.perks_for_guest_of_honor}
                  onChange={handleInputChange}
                  rows={3}
                  className="w-full rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                  placeholder="Special perks and benefits for the guest of honor..."
                />
              </div>
              ...
              {formData.balloon_package_included && (
                <>
                  <div className="md:col-span-3">
                    <div className="p-4 bg-pink-50 rounded-lg border border-pink-200">
                      <h4 className="text-lg font-semibold text-pink-800 mb-4">Balloon Package Configuration (Optional)</h4>
                      ...
                      {/* Balloon Package Selection - NOW OPTIONAL */}
                      <div className="mb-4">
                        <label className="block text-sm font-medium text-pink-700 mb-2">
                          Select Balloon Package (Optional)
                        </label>
                        <div className="flex space-x-4 items-center">
                          ...
                          >
                            {formData.balloon_party_package ? 'Change Balloon Package' : 'Select Balloon Package'}
                          </button>
                          ...
                          {formData.balloon_party_package && (
                            <button
                              ...
                              className="text-sm text-red-600 hover:text-red-800 px-3 py-1 border border-red-200 rounded hover:border-red-300"
                            >
                              Clear Selection
                            </button>
                          )}
                        ...
                        <p className="text-xs text-gray-500 mt-2">
                          Optional: You can include a balloon package with this birthday package. Leave unselected if not needed.
                        </p>
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500 mt-2">
                          Optional credit amount for balloon package.
                        </p>
                      </div>
                    </div>
                  </div>
